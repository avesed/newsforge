...
async def _clean_content(raw_text: str, title: str) -> tuple[str, int]:
    ...
    task_instructions = (
        ...
        "保留：文章正文段落、数据表格、引用块、正文内嵌的人名提及、正文配图。\n\n"
        "图片规则（最高优先级）：\n"
        "- 所有 ![...](...) 格式的Markdown图片引用默认保留，除非URL明显是logo/图标/占位符\n"
        "- 图片版权标注处理：仅处理原文中已有的独立版权文字（如图片旁边或下方出现的\"Getty Images\"、\"Reuters\"等），"
        "将其格式化为图片下方斜体。不要根据alt text猜测或添加版权标注\n"
        "  例：![Getty Images 描述文字](url)Getty Images → ![描述文字](url)\\n*Getty Images*\n"
        "  例：![描述](url)\\n图/路透 → ![描述](url)\\n*路透*\n"
        "  反例：![blake lively getty](url)（alt text含getty但无独立版权文字）→ 原样保留，不要添加*Getty Images*\n"
        "- 仅移除这些图片：URL以data:开头、含placeholder、或明显是1x1追踪像素\n"
        "- 严禁编造、替换或生成任何图片URL\n\n"
