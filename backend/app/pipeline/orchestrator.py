...
async def _clean_content(raw_text: str, title: str) -> tuple[str, int]:
    ...
    task_instructions = (
        ...
        "数据整理规则：\n"
        "- 如果正文中出现**零散排列的键值对数据**（如股票行情：指标名换行后跟数值，反复出现），"
        "将其整理为Markdown表格，表头为指标名，数据为对应值\n"
        "- 仅对明显的结构化数据（财务指标、行情数据、规格参数等）做此整理，叙述性段落不动\n"
        "- 表格中的数据必须与原文完全一致，不得增删改\n\n"
        "图片规则（最高优先级，必须严格遵守）：\n"
...
async def run_pipeline(article_id: str, article_data: dict, progress_callback=None) -> dict[str, Any]:
    ...
    # Look up source/feed categories + finance_metadata for trigger logic
    ...
    try:
        ...
        async with _factory() as _sess:
            ...
            _row = await _sess.execute(_text(
                "SELECT c.slug, a.finance_metadata FROM articles a "
            ...
            if _r:
                if _r[0]:
                    source_categories = [_r[0]]
                if isinstance(_r[1], dict) and _r[1].get("stockpulse_source"):
                    is_stockpulse = True
    except Exception:
