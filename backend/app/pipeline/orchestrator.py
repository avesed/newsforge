...
async def _clean_content(raw_text: str, title: str) -> tuple[str, int]:
    ...
    task_instructions = (
        ...
        "视频/音频播放器相关文字（如播放按钮文本、时间戳\"00:00\"、"
        "\"Play\"、\"Watch\"、\"Next video\"、\"分享到\"等控件文字）。\n"
        "保留：文章正文、数据表格、引用块。\n\n"
        "图片规则：\n"
        "- 仅保留与正文内容相关的配图，必须使用原文中的真实完整URL\n"
        "- 移除：网站logo、图标、广告图、占位符图（data:开头或含placeholder的URL）、"
        "追踪像素（1x1图片）、表情图标\n"
        "- 严禁编造、替换或生成任何图片URL。如果不确定图片是否属于正文配图则移除\n\n"
